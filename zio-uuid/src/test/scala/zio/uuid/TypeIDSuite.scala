...
import zio.prelude.{DebugOps, Equal, Validation}
...
class TypeIDSuite extends FunSuite {
  ...
  test("TypeID should not decode invalid typeIDs") {
    ...
    // prefix-64-chars
    assertInvalidDecoding(typeID = "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijkl_00000000000000000000000000")
  ...
  test("TypeID should have an Eq instance") {
    val eq = implicitly[Equal[TypeID]]
    assert(
      eq.equal(
  ...
  test("TypeID should have a Debug instance") {
    assertEquals(
      TypeID.decode("prefix_01h455vb4pex5vsknk084sn02q").toOption.get.debug.render(Renderer.Scala),
  ...
  private def assertValidDecoding(
    ...
    val expected = TypeID
      .build(prefix = prefix, uuid = uuid, uuid.version() == 7)
      .mapError(_ => TypeID.DecodeError.InvalidTypeID("Test"): TypeID.DecodeError)
